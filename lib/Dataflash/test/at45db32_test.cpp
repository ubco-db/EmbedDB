...
static char *test_buffer_read_write_buffer_1()
{
    ...
    //erase page 0
    df_page_erase(&at45db32_m, 1);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };

    //read into buffer 1
    df_MM_to_buffer_1(&at45db32_m, 1);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };

    //check for 0
    uint8_t temp_data[528];
    df_buffer_1_read(&at45db32_m, 0, temp_data, 528);
    for (uint16_t i = 0; i < 528; i++)
    {
#ifdef SHOW_OUTOUT
    ...
    df_buffer_1_write(&at45db32_m, 0, data, 528);
    ...
    {
        //knockout cell
        df_buffer_1_write(&at45db32_m, i, &cell, 1);
        //read back and compare
        data[i] = cell;

        df_buffer_1_read(&at45db32_m, 0, temp_data, 528);

        for (uint16_t j = 0; j < 528; j++)
        {
#ifdef SHOW_OUTPUT
...
static char *test_buffer_read_write_buffer_2()
{
    ...
    //read into buffer 1
    df_MM_to_buffer_2(&at45db32_m, 1);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
    ...
    //check for 0
    uint8_t temp_data[528];
    df_buffer_2_read(&at45db32_m, 0, temp_data, 528);
    //Serial.println("\nErase data");
    for (uint16_t i = 0; i < 528; i++)
    {
#ifdef SHOW_OUTOUT
    ...
    //write data to buffer
    df_buffer_2_write(&at45db32_m, 0, data, 528);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
...
//Serial.println("\nRead back");
    ...
    {
        //knockout cell
        df_buffer_2_write(&at45db32_m, i, &cell, 1);
        //read back and compare
        data[i] = cell;
...
static char *test_write_to_MM_page_w_erase_buffer_1()
{
    ...
    df_buffer_1_write(&at45db32_m, 0, data, 528);
    df_buffer_1_to_MM_erase(&at45db32_m, 0);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
    //read back to buffer 2 and compare
    df_MM_to_buffer_2(&at45db32_m, 0);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
...
static char *test_write_to_MM_page_w_erase_buffer_2()
{
    ...
    //write to buffer 1 and store in page
    df_buffer_2_write(&at45db32_m, 0, data, 528);
    df_buffer_2_to_MM_erase(&at45db32_m, 0);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
    //read back to buffer 2 and compare
    df_MM_to_buffer_1(&at45db32_m, 0);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
...
static char *test_write_to_MM_page_wo_erase_buffer_1()
{
    ...
    //write to buffer 1 and store in page
    df_buffer_1_write(&at45db32_m, 0, data, 528);
    df_buffer_1_to_MM_no_erase(&at45db32_m, 0);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
...
static char *test_write_to_MM_page_wo_erase_buffer_2()
{
    ...
    //write to buffer 1 and store in page
    df_buffer_2_write(&at45db32_m, 0, data, 528);
    df_buffer_2_to_MM_no_erase(&at45db32_m, 0);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
...
static char *test_MM_direct_read()
{
    ...
    uint8_t temp_data[528];
    df_main_memory_read(&at45db32_m, 0, 0, temp_data, 528);
    //check buffer compare method
    for (uint16_t j = 0; j < 528; j++)
    {
#ifdef SHOW_OUTPUT
...
static char *test_MM_direct_read_partial()
{
    ...
    //write to buffer 1 and store in page
    df_buffer_1_write(&at45db32_m, 0, data, 528);
    //update partial record - watch out for wrapping - checks only the 528 byte page
    //Data should wrap around
    for (uint16_t position = 0; position < 528; position++)
    {
        df_buffer_1_write(&at45db32_m, position, small_data, 4);
        df_buffer_1_to_MM_erase(&at45db32_m, 0);
        //need to have a wait state here!
        while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
        {
        ...
        df_main_memory_read(&at45db32_m, 0, position, temp_data, 4);
        //check buffer compare method
...
static char *test_MM_memory_comparison()
{
    ...
    for (uint16_t position = 0; position < 528; position++)
    {
        ...
        df_compare_buffer_1_to_MM(&at45db32_m, 0);
        while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
        {
        };

        mu_assert("error, bad data on compare", DATAFLASH_BUFFERS_MATCH == get_page_memory_comparision(&at45db32_m));
    }
    return 0;
}
...
static char *test_page_access()
{
    ...
    for (uint16_t page = 0; page < 8192; page++)
    {
        ...
        df_buffer_1_write(&at45db32_m, 0, data, 528);
        df_buffer_1_to_MM_no_erase(&at45db32_m, page);
        //need to have a wait state here!
        while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
        {
        };
        ...
        df_main_memory_read(&at45db32_m, page, 0, temp_data, 528);
...
static char *test_address_translation()
{
    ...
    //8192 pageds for this device

    /** 
    * For a 528 byte page, the address is composed of:
    *       -one dummy bit
    *       -13 page address bits (MSB -> LSB)
    *       -10 dummy bits
    *
    * For a 512 byte page, the address is composed of:
    *       -two dummy bits
    *       -13 page address bits (MSB -> LSB)
    *       -9 dummy bits
    */
    ...
    test_memory.page_size = 512;
    test_memory.bits_per_page = 10;
    test_memory.actual_page_size = 528;
    ...
    for (test_address = 0; test_address < 8192; test_address++)
    {
        ...
        *((uint16_t *)address_array) = (*((uint16_t *)address_array) << 2); //(byte 0 and 1 contain address shifted by 2)
    ...
    test_memory.actual_page_size = 512;
...
static char *test_continious_read_lf()
{
    Serial.println(__func__);
    //input dataset
    uint8_t large_data[528 * 4];

    for (uint16_t i = 0; i < 528 * 4; i++)
    {
    ...
    df_buffer_2_write(&at45db32_m, 0, large_data, 528);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
    ...
    df_buffer_2_write(&at45db32_m, 0, (large_data + 528), 528);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };

    df_buffer_2_to_MM_erase(&at45db32_m, 1);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
    ...
    df_buffer_2_write(&at45db32_m, 0, (large_data + (2 * 528)), 528);
    df_buffer_2_to_MM_erase(&at45db32_m, 2);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };
    ...
    df_buffer_2_write(&at45db32_m, 0, (large_data + (3 * 528)), 528);
    df_buffer_2_to_MM_erase(&at45db32_m, 3);
    //need to have a wait state here!
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };

    uint8_t temp_data[4 * 528];
    df_continious_array_read_lf(&at45db32_m, 0, 0, temp_data, 4 * 528);
    //df_main_memory_read(&at45db32_m,0,0,temp_data,528);
    while (DATAFLASH_BUSY == get_ready_status(&at45db32_m))
    {
    };

    //df_main_memory_read(&at45db32_m,1,0,(temp_data+528),528);
    //while (DATAFLASH_BUSY == get_ready_status(&at45db32_m)){};

    //check buffer compare method
    for (uint16_t j = 0; j < (4 * 528); j++)
    {
#ifdef SHOW_OUTPUT
        Serial.print(j);
        Serial.print(":");
...
static char *check_device_id()
{
    ...
    df_get_device_id(&at45db32_m, data);
    ...
    for (uint8_t i = 0; i < 5; i++) //5 bytes for id
    {
        ...
        Serial.print(i);
        Serial.print(":");
        Serial.print(at45db32_m.device_id[i]);
        Serial.print(":");
        Serial.print(data[i]);
        Serial.print(" ");
#endif
        mu_assert("error, bad data on compare", at45db32_m.device_id[i] == data[i]);
    }
