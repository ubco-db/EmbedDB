...
uint8_t loadRowData(orderByInfo *state, embedDBOperator *op, void *outputFile, uint16_t recordSize);
...
void initSort(embedDBOperator* op) {
    ...
    // Output rows to file
    count = loadRowData(state, op, rowData, recordSize);
...
uint8_t loadRowData(orderByInfo *state, embedDBOperator *op, void *outputFile, uint16_t recordSize) {
    ...
    while (exec(op->input)) {
        // Write row data
        if (state->fileInterface->write(op->input->recordBuffer, count++, recordSize, outputFile) == 0) {
            #ifdef PRINT_ERRORS
...
uint8_t readNextRecord(file_iterator_state_t *state, void *buffer) {
    ...
    // Copy result ignoring key
    memcpy(buffer, (uint8_t *)state->readBuffer + sizeof(int32_t), state->recordSize - sizeof(int32_t));
