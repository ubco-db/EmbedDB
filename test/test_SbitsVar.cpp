...
void setupBoard_varTest() {
    ...
    if (test_sd_card()) {
        file_varTest = sd_varTest.open("/");
        cout << F("\nList of files on the SD.\n");
        sd_varTest.ls("/", LS_R);
    }

    init_sdcard((void *)&sd_varTest);
...
int runUnityTests_varTest() {
    UNITY_BEGIN();

    for (i = 0; (unsigned int)i < sizeof(dataSizes) / sizeof(dataSizes[i]); i++) {
        // Setup state
        initState(dataSizes[i]);
        RUN_TEST(test_init_varTest);
...
void setup() {
    setupBoard_varTest();
    runUnityTests_varTest();
}

void loop() {}

bool test_sd_card_varTest() {
    if (!sd_varTest.cardBegin(SD_CONFIG)) {
        Serial.println(F(
        ...
        errorPrint(sd_varTest);
        return false;
    }

    if (!sd_varTest.card()->readCID(&m_cid) ||
        !sd_varTest.card()->readCSD(&m_csd) ||
        !sd_varTest.card()->readOCR(&m_ocr)) {
        cout << F("readInfo failed\n");
        errorPrint(sd_varTest);
    }
    printCardType(sd_varTest);
    cidDmp();
    csdDmp();
    cout << F("\nOCR: ") << uppercase << showbase;
    cout << hex << m_ocr << dec << endl;
    if (!mbrDmp(sd_varTest)) {
        return false;
    }
    if (!sd_varTest.volumeBegin()) {
